...
sealed class Node {
    ...
    class ModuleBuilder {
        val types: MutableList<Type.Func> = mutableListOf()
        val imports: MutableList<Import> = mutableListOf()
        val tables: List<Type.Table> = arrayListOf()
        val memories: List<Type.Memory> = arrayListOf()
        val globals: List<Global> = arrayListOf()
        val exports: MutableList<Export> = mutableListOf()
        val startFuncIndex: Int? = null
        val elems: List<Elem> = arrayListOf()
        val funcs: MutableList<Func> = mutableListOf()
        val data: MutableList<Data> = mutableListOf()
        var names: NameSection? = null
        val customSections: List<CustomSection> = arrayListOf()


        fun create(): Module {
            val memories1 = arrayListOf(Type.Memory(ResizableLimits(1, 2)))
            return Module(types, imports, tables, memories1, globals, exports, startFuncIndex, elems, funcs, data, names, customSections)
        }

        fun withFunctions(funcs: List<Func>): ModuleBuilder {
            this.funcs.addAll(funcs)
            return this
        }

        fun withNames(nameSection: NameSection?): ModuleBuilder {
            this.names = nameSection
            return this
        }

        fun withData(data: Data): ModuleBuilder {
            this.data.add(data)
            return this
        }

        fun withExports(exports: List<Export>): ModuleBuilder {
            this.exports.addAll(exports)
            return this
        }

        fun withImports(imports: List<Import>): ModuleBuilder {
            this.imports.addAll(imports)
            return this
        }

        fun withTypes(types: List<Type.Func>): ModuleBuilder {
    ...
    sealed class InstrOp<out A : Instr.Args> {
        ...
        companion object {
            ...
            init {
                ...
                opMapEntry("local.get", 0x20, VarOp::IndexArg, Instr::GetLocal, Instr.GetLocal::class)
                opMapEntry("local.set", 0x21, VarOp::IndexArg, Instr::SetLocal, Instr.SetLocal::class)
                opMapEntry("local.tee", 0x22, VarOp::IndexArg, Instr::TeeLocal, Instr.TeeLocal::class)
                opMapEntry("global.get", 0x23, VarOp::IndexArg, Instr::GetGlobal, Instr.GetGlobal::class)
                opMapEntry("global.set", 0x24, VarOp::IndexArg, Instr::SetGlobal, Instr.SetGlobal::class)
