...
class RunModule(override val name: String?, val ctx: Interpreter.Context) : Module {
  ...
  override fun exportedGlobal(field: String) = ctx.exportIndex(field, Node.ExternalKind.GLOBAL)?.let { index ->
    ...
    var getter = lookup.bind(ctx, "getGlobal", MethodType.methodType(Number::class.java, Int::class.javaPrimitiveType))
    var setter = if (!type.mutable) {
      null
    } else {
      lookup.bind(ctx, "setGlobal", MethodType.methodType(Void::class.javaPrimitiveType, Int::class.javaPrimitiveType, Number::class.java)) // Cast number to specific type
    }
    getter = MethodHandles.explicitCastArguments(getter, MethodType.methodType(type.contentType.jclass, Int::class.javaPrimitiveType))
    if (setter != null) {
      setter = MethodHandles.explicitCastArguments(setter, MethodType.methodType(Void::class.javaPrimitiveType, Int::class.javaPrimitiveType, type.contentType.jclass))
    }
  ...
  class Builder(
    ...
    override fun build(imports: Module.ImportResolver, mod: Node.Module, name: String?): RunModule {
      val context = Interpreter.Context(
              mod = mod,
              logger = logger,
      ...
      mod.startFuncIndex?.also { Interpreter.execFunc(context, it) }
      return RunModule(name = name, ctx = context)
    }
  }
